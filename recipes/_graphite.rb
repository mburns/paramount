...
# https://github.com/sous-chefs/graphite/blob/master/test/fixtures/cookbooks/test/recipes/single_node.rb
...
graphite_secret_key = random_password
Chef::Log.info("Graphite Secret Key: #{graphite_secret_key}")

base_dir = (node['graphite']['base_dir']).to_s

graphite_web_config "#{base_dir}/webapp/graphite/local_settings.py" do
  config(secret_key: graphite_secret_key,
...
graphite_passwd = random_password
Chef::Log.info("Graphite Password: #{graphite_passwd}")
node.normal['graphite']['password'] = graphite_passwd
